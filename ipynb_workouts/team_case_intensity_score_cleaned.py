...
icd_scores= pd.read_csv("icd10_diagnoses_scored.csv")
...
df['case_response_time'] = round((pd.to_datetime(df['Olay Yeri Varış Tarih Saat']) - pd.to_datetime(df['İhbar/Çağrı Tarih Saat'])).dt.total_seconds() / 60, 2)  # Convert to minutes
...
df.loc[(df['case_response_time']>1440) | (df['case_response_time']<0), 'case_response_time']= np.nan
...
team_population_densities= df_district_case_counts.groupby(['Ekip No']).agg({'case_count': 'sum', 'teams_population_density': 'mean'}).reset_index().sort_values(by='teams_population_density', ascending=False)
        ...
        if not subset.empty:
            ...
            for col in df_team_grouped.columns:
                if col.endswith('_logarithmic_score'):
                    ...
                    df_team_grouped[new_col] = (df_team_grouped[col] - df_team_grouped[col].mean()) / df_team_grouped[col].std()
            ...
            df_team_grouped['total_score_z']= (df_team_grouped['total_score'] - df_team_grouped['total_score'].mean()) / df_team_grouped['total_score'].std()
            df_team_grouped= df_team_grouped.sort_values(by='total_score_z', ascending=False).reset_index()

            df_team_grouped.to_excel(rf"C:\Users\mkaya\OneDrive\Masaüstü\istanbul112_hidden\data\case_reports\europe\parquet_files\team_case_intensities\{season}_{day}_total scores.xlsx")
